...
-- ============================================================
-- TABLAS ADICIONALES (Recuperadas de producción)
-- ============================================================
...
CREATE POLICY "Acceso a clientes" ON public.clientes FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE TABLE IF NOT EXISTS public.cuadraturas (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  created_at timestamptz DEFAULT timezone('utc', now()) NOT NULL
);
ALTER TABLE public.cuadraturas ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Acceso a cuadraturas" ON public.cuadraturas FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE TABLE IF NOT EXISTS public.mantencion_prepagada (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  cuadratura_id bigint REFERENCES public.cuadraturas(id) ON DELETE CASCADE,
  mantencion_10000 boolean DEFAULT false,
  mantencion_20000 boolean DEFAULT false,
  mantencion_30000 boolean DEFAULT false,
  created_at timestamptz DEFAULT timezone('utc', now()) NOT NULL
);
...
ALTER TABLE public.negocios_validaciones ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Acceso a validaciones" ON public.negocios_validaciones FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE TABLE IF NOT EXISTS public.negocios_historial (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  pedido_venta text REFERENCES public.negocios(pedido_venta) ON DELETE CASCADE,
  tipo_evento text,
  descripcion text,
...
ALTER TABLE public.negocios_historial ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Acceso a historial" ON public.negocios_historial FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE TABLE IF NOT EXISTS public.calculo_pc (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  created_at timestamptz DEFAULT timezone('utc', now()) NOT NULL
);
ALTER TABLE public.calculo_pc ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Acceso a calculo_pc" ON public.calculo_pc FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
CREATE POLICY "Acceso a cuadratura_valores_cliente" ON public.cuadratura_valores_cliente FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE TABLE IF NOT EXISTS public.cuadratura_pagos (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  id_cuadratura bigint REFERENCES public.cuadraturas(id) ON DELETE CASCADE,
  created_at timestamptz DEFAULT timezone('utc', now()) NOT NULL
);
ALTER TABLE public.cuadratura_pagos ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Acceso a cuadratura_pagos" ON public.cuadratura_pagos FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
CREATE POLICY "Acceso a sucursales" ON public.sucursales FOR ALL TO authenticated USING (true) WITH CHECK (true);
